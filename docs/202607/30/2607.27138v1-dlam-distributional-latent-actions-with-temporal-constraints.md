...
score: 6.0
evidence: 带时间约束的分布潜动作模型用于世界模型
tldr: 现有VLA模型受限于动作标签数据稀缺，无动作视频虽蕴含丰富物理变化，但潜在动作模型缺乏与机器人动作联合生成所需的结构。DLAM将每步转移建模为对角高斯分布，通过归一化组合和反转约束均值和方差，学习时间一致的潜在动力学。在保留视频重建和下游策略任务上超越基线，显著提升MetaWorld MT50、LIBERO和真实操纵任务性能。贡献在于分布化潜在表示与结构化时间约束的有效融合。
source: arxiv
...
motivation: 动作标签数据稀缺，需从无动作视频中学习时间一致的潜在动力学以辅助策略学习。
method: 将每步转移表示为对角高斯，用归一化组合和反转约束均值和方差，并用flow-matching策略联合生成转移序列与动作。
result: 在保留视频重建和下游策略任务上优于基线，显著提升MetaWorld、LIBERO和真实操纵任务性能。
conclusion: 分布化潜在动作模型通过时间约束学习一致动力学，有效提升策略学习效果。
---

## 摘要
视觉-语言-动作模型仍受限于稀缺的动作标注机器人数据，而无动作视频提供了丰富的物理变化观测。潜在动作模型可以提取此类先验，但基于重建训练的编码在预测未来观测时可能缺乏与机器人动作联合生成所需的结构。现有结构化方法添加了时序约束，但仍保留确定性转移点，因此局部推断的转移中的残差误差可能在递归组合下传播并累积。我们提出DLAM，一种分布潜在动作模型，将每个转移表示为对角高斯分布。以参考帧为条件的重建将均值锚定在观测到的视觉变化上，而等间隔三元组上的归一化组合与反转约束了均值和逐维方差。方差组合使用轻量级共享相关系数来解释共享中间帧的相邻转移之间的依赖关系，而反转则对均值取负并保持方差不变。对于下游策略学习，我们冻结编码器并训练一个流匹配策略来联合生成均值转移序列和机器人动作。在保留的转移上，DLAM学习到比现有潜在动作基线更时序一致的潜在动态，并在保留视频上实现更强的直接和累积重建。在相同受控的π₀迁移协议下，它还提高了MetaWorld MT50、LIBERO和真实世界操作任务上的策略性能。控制消融实验表明，归一化均值约束贡献了大部分重建增益，而学习到的方差和相关性感知的组合在下游控制中提供了互补的改进。
