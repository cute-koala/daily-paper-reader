...
score: 6.0
evidence: 随机多目标动力学规划
tldr: 针对含随机混合对手环境的多目标运动规划问题，本文提出将规划空间转向闭环策略序列，并通过蒙特卡洛粒子rollout将样本风险评估直接融入树构建。算法SMO-RRT具有概率完备性，SMO-SST通过选择性剪枝提升数值性能。推导了非高斯、状态依赖不确定性下机会约束违反概率的有限样本界，为多智能体系统、社交导航和自动驾驶等场景提供了可证明安全的规划方法。
source: arxiv
...
motivation: 现有机会约束规划器基于开环轨迹评估风险，过于保守且未考虑自主体反应性，难以应对随机混合对手。
method: 将规划空间扩展为闭环策略序列，通过蒙特卡洛粒子rollout将样本风险评估融入RRT树构建，提出SMO-RRT与SMO-SST。
result: SMO-RRT证明概率完备性，SMO-SST通过选择性剪枝提升性能；导出有限样本机会约束违反概率上界。
conclusion: 该方法适用于多智能体、社交导航和自动驾驶，实现可证明安全的随机多目标规划。
---

## 摘要
本文研究在具有随机混合对手的环境中进行的多目标运动学动力学规划，这些对手根据自我状态以概率方式转换至对抗模式。目标是在执行成本与安全约束违反概率（风险）之间进行权衡，构建帕累托前沿路径。现有的机会约束规划器在开环轨迹上评估风险，产生过于保守的解，未能考虑自主体反应性。为解决此局限，我们将规划空间转移到闭环策略序列，并通过蒙特卡洛粒子推演将基于样本的风险评估直接集成到树构建中。首先引入随机多目标RRT（SMO-RRT），我们证明其概率完备性；随后提出随机多目标稳定稀疏RRT（SMO-SST），该方法利用选择性剪枝以牺牲完备性为代价提高数值性能。对于这两种算法，我们推导出具有非高斯、状态相关不确定性系统的机会约束违反概率的有限样本界，从而在适用于多智能体系统、社交导航和自动驾驶的广泛环境中实现概率安全规划。
