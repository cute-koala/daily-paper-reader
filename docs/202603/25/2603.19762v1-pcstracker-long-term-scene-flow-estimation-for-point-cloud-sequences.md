...
result: 在PointOdyssey3D和ADT3D数据集上取得领先精度，且推理速度达到32.5 FPS，性能优于现有RGB-D方案。
conclusion: 该研究为长序列点云运动分析提供了高效且一致的解决方案，证明了端到端时空建模在3D场景流中的优越性。
---

## 摘要
点云场景流估计是长期和细粒度 3D 运动分析的基础。然而，现有方法通常局限于两帧设置，且随着几何演变、遮挡出现和误差累积，难以在长序列中保持时间一致性。在这项工作中，我们提出了 PCSTracker，这是首个专门为点云序列中一致场景流估计设计的端到端框架。具体而言，我们引入了一个迭代几何运动联合优化模块（IGMO），该模块显式地建模点特征的时间演变，以缓解由动态几何变化引起的对应关系不一致。此外，我们提出了一个时空点轨迹更新模块（STTU），利用广泛的时间上下文来推断遮挡点的合理位置，确保连贯的运动估计。为了进一步处理长序列，我们采用了一种重叠滑动窗口推理策略，交替进行跨窗口传播和窗口内细化，从而有效抑制误差累积并保持稳定的长期运动一致性。在合成数据集 PointOdyssey3D 和真实世界数据集 ADT3D 上的广泛实验表明，PCSTracker 在长期场景流估计中达到了最佳精度，并保持了 32.5 FPS 的实时性能，同时与基于 RGB-D 的方法相比，展示了卓越的 3D 运动理解能力。
