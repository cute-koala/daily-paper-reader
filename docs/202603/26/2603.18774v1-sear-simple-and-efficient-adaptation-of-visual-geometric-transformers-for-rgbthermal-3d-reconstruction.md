...
## 摘要
基础前馈视觉几何模型通过从大规模 RGB 数据集中学习强大的场景先验，实现了准确且高效的相机位姿估计和场景重建。然而，当应用于混合感知模态（如 RGB-热成像 (RGB-T) 图像）时，其有效性会有所下降。我们观察到，虽然在 RGB 数据上预训练的视觉几何 Transformer 在仅热成像的重建中具有良好的泛化能力，但在联合处理 RGB 和热成像模态时，难以实现两者的对齐。为解决这一问题，我们提出了 SEAR，这是一种简单且高效的微调策略，可将预训练的几何 Transformer 适配到多模态 RGB-T 输入。尽管仅在相对较小的 RGB-T 数据集上进行训练，我们的方法在 3D 重建和相机位姿估计方面仍显著优于现有最先进的方法，在所有指标上均取得了重大改进（例如 AUC@30 提升超过 29%），并且与原始预训练模型相比，在推理时间开销几乎可以忽略不计的情况下，提供了更高的细节度和模态间的一致性。值得注意的是，SEAR 即使在低光照和浓烟等挑战性条件下，也能实现可靠的多模态位姿估计和重建。我们通过广泛的消融实验验证了我们的架构，展示了模型如何对齐两种模态。此外，我们还引入了一个新数据集，包含在不同时间、视角和光照条件下采集的 RGB 和热成像序列，为多模态 3D 场景重建的未来工作提供了可靠的基准。代码和模型已在 https://www.github.com/Schindler-EPFL-Lab/SEAR 公开。
