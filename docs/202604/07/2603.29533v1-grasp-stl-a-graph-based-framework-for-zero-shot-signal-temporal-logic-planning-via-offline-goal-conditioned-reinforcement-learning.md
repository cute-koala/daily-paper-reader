...
tldr: 本文研究在仅有离线数据集且无环境交互的条件下，实现零样本信号时序逻辑（STL）规划的问题。为此提出了GraSP-STL框架：通过学习目标条件价值函数诱导状态空间的可达性度量，并构建有向图抽象；将规划转化为基于鲁棒性语义的图搜索问题。该方法分离了可重用的可达性学习与任务条件规划，实现了对未见STL任务的零样本泛化及长视距规划。
source: arxiv
...
motivation: 研究在仅有任务无关行为策略收集的离线数据集、无环境交互或任务特定重训练的情况下，如何合成满足任意未见信号时序逻辑（STL）规范的控制策略。
method: 提出GraSP-STL框架：从离线数据学习目标条件价值函数以诱导可达性度量，构建有向图抽象，将规划转化为基于算术几何平均鲁棒性的图搜索问题。
result: 实验结果表明，该方法在一系列离线STL规划任务上有效。
conclusion: 该框架将可重用的可达性学习与任务条件规划分离，实现了对未见STL任务的零样本泛化，并通过离线数据中短视距行为的组合完成长视距规划。
---

## 摘要
本文研究了在信号时序逻辑（STL）规范下的离线零样本规划问题。我们假设仅能访问一个与任务无关的行为策略所收集的状态-动作-状态转移离线数据集，没有解析动力学模型，没有进一步的环境交互，也没有针对特定任务的重新训练。目标是合成一个控制策略，其产生的轨迹满足任意未见过的STL规范。为此，我们提出了GraSP-STL，一个基于图搜索的框架，用于从离线轨迹进行零样本STL规划。该方法从离线数据中学习一个目标条件价值函数，并利用它在状态空间上诱导出一个有限时域可达性度量。基于此度量，它构建一个有向图抽象，其节点代表代表性状态，边编码可行的短时域转移。规划随后被表述为对航点序列的图搜索过程，使用算术-几何平均鲁棒性及其区间语义进行评估，并通过学习的目标条件策略执行。所提出的框架将可重用的可达性学习与任务条件规划分离，从而能够通过组合离线数据中的短时域行为来实现对未见STL任务的零样本泛化和长时域规划。实验结果证明了其在一系列离线STL规划任务上的有效性。
