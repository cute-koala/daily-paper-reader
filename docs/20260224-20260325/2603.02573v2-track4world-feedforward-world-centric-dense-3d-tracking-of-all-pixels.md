...
method: 提出一种基于 VGGT 风格 ViT 的前馈模型，利用 3D 相关方案同时估计任意帧间的像素级 2D/3D 密集流，并结合重建的几何信息实现世界坐标系下的 3D 跟踪。
result: 在多个基准测试中，该方法在 2D/3D 流估计和 3D 跟踪性能上一致优于现有技术。
conclusion: Track4World 为现实世界的 4D 重建任务提供了一个高效、稳健且具备扩展性的全像素 3D 动态理解框架。
---
